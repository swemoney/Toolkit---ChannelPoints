        ...
        public override void ParseMessage(ITwitchMessage msg)
        {
            ...
            if (msg.ChatMessage.CustomRewardId != null)
            {
                ...
                else
                {
                    ...
                    {
                        Helper.LogMessage($"Detected a custom reward that wasn't configured: {rewardId}");
                    }
                    ...
                    if (autoReward != null)
                    {
                        if (ChannelPoints_Settings.ShowDebugMessages)
                        {
                            Helper.LogMessage($"A reward with Automatic UUID capture enabled was found. Configuring this reward to use {rewardId}.");
                        }
                    ...
                    else
                    {
                        if (ChannelPoints_Settings.ShowDebugMessages)
                        {
                            Helper.LogMessage("If this is the reward you would like to use, add this UUID to the mod settings.");
                        }
                    }
                }
            }
